...
module.exports = class RankCard {
    ...
    setForgroundColor(color) {
        if (color) {
            if (/^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(color)) {
                ...
                throw "The argument of setForgoundColor method must be a hexadecimal color."
            }
        }
    }

    setForgroundOpacity(opacity = 0) {
        if (opacity) {
            if (opacity >= 0 && opacity <= 1) {
                this.foreground.opacity = opacity;
                return this;
            } else {
                throw new Error("The value of the opacity of setForgroundOpacity method must be between 0 and 1 (0 and 1 included).");
            }
        }
    }
    ...
    setDiscriminator(discriminator, color = "#fff") {
        this.discriminator.data = discriminator.substr(0, 4);
        if (color) {
            if (/^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(color)) {
